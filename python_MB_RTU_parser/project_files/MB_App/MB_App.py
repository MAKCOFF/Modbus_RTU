...
class MBScraper(client_RTU):
    ...
    def read(self):
        ...
        self.result = [self.data_result, fact_reg, self.traceback_error, err_cnt]
        return self.result
        ...
        match self.mode_read_registers:
            case 1:
                print("Запрошено", len(self.data_result) - 1,
                      ...
                      self.data_result)
                if err_cnt > 0:
                    print("   !pymodbus:\terr_cnt: %s; last tb: %s" % (err_cnt, self.traceback_error))
            case 2:
                print("Запрошено", len(self.data_result) - 1,
                      "регистров по одному(size 2 BYTE) за каждый запрос \n",
            ...
            case 3:
                print("Запрошено", len(self.data_result) - 1,
                      ...
                      self.data_result)
                if err_cnt > 0:
                    print("   !pymodbus:\terrCnt: %s; last tb: %s" % (err_cnt, self.traceback_error))
            case 4:
                print("Запрошено", len(self.data_result) - 1,
                      "регистров по одному(size 1 BIT) за каждый запрос \n",
